...
# Kullanıcı dil tercihlerini saklamak (user_id -> "en" veya "tr")
user_language = {}
...
def generate_signals(indicators: dict) -> dict:
    ...
    rsi_condition_long = rsi_value < 70
    rsi_condition_short = rsi_value > 30
    buy_signal_strength = (data['macd'].iloc[-1] - data['macd_signal'].iloc[-1]) if (current_price > pivot_points['pivot'] and ema_condition_long and obv_condition_long and rsi_condition_long) else None
    sell_signal_strength = (data['macd_signal'].iloc[-1] - data['macd'].iloc[-1]) if (current_price < pivot_points['pivot'] and ema_condition_short and obv_condition_short and rsi_condition_short) else None
...
# Mapping: short -> "1h", medium -> "4h", long -> "1d"
...
async def coin_analysis_by_term(symbol: str, term: str, lang: str, username: str) -> (str, io.BytesIO):
    timeframe = map_term_to_interval(term)
    indicators = await get_technical_indicators(symbol, timeframe)
    if not indicators:
        return (f"⚠️ {symbol} - sembolü kontrol edin veya daha sonra tekrar deneyin.", None)
    signals = generate_signals(indicators)
    signal_message = f"🪬 *{symbol} ({term.capitalize()} Term)*\n\n"
    ...
    risk_setting = user_risk_settings.get(username, None)
    ...
    return signal_message, chart_image

async def long_signals_by_term(term: str, lang: str) -> str:
    timeframe = map_term_to_interval(term)
    symbols = TARGET_COINS
    long_signals_list = []
    tasks = [get_technical_indicators(symbol, timeframe) for symbol in symbols]
    ...
    if long_signals_list:
        ...
        signals_message = f"🚀 *{t('top_long_signals', lang)} ({term.capitalize()} Term)*\n\n"
    ...
    for symbol, indicators in zip(symbols, results):
        if indicators:
            ...
            if signals['sell_signal']:
                short_signals_list.append((symbol, signals['sell_signal_strength'], signals, indicators['entry_price']))
    if short_signals_list:
        short_signals_list = sorted(short_signals_list, key=lambda x: x[1], reverse=True)[:7]
        signals_message = f"🩸 *{t('top_short_signals', lang)} ({term.capitalize()} Term)*\n\n"
        for symbol, strength, sig, entry_price in short_signals_list:
            signals_message += (
                f"🪬*{symbol}*\n\n"
                f"   {t('direction', lang)}: {t('short', lang)}\n"
                f"   {t('entry', lang)}: {format_price(entry_price)}\n"
...
async def trend_analysis_by_term(term: str, lang: str) -> (str, io.BytesIO):
    # For trend analysis, we adjust BTC data timeframe accordingly.
    interval = map_term_to_interval(term)
    fng_index = None
    try:
        async with aiohttp.ClientSession() as session:
    ...
    btc_ema_50 = None
    try:
        btc_data = await get_crypto_data("BTCUSDT", interval=interval, limit=60)
    ...
    if fng_index is not None and btc_ema_20 is not None and btc_ema_50 is not None:
        if fng_index > 60 and btc_ema_20 > btc_ema_50:
            trend_result = "TREND Long 🚀"
        elif fng_index < 40 and btc_ema_20 < btc_ema_50:
            trend_result = "TREND Short 🩸"
        else:
            trend_result = "TREND Nötr 🤚🏼"
    else:
        trend_result = "Trend analizi için gerekli veriler alınamadı."
    trend_chart = await generate_trend_chart_custom(interval)
    return trend_result, trend_chart
...
async def get_market_news(lang: str) -> list:
    if lang == "tr":
        ...
        news_url = f"https://newsapi.org/v2/everything?q={query}&language=tr&sortBy=publishedAt&apiKey={NEWS_API_KEY}"
        published_text = "Yayınlandı:"
                    ...
                    if response.status == 200:
                        ...
                        for article in articles[:5]:
                            ...
                            try:
                                ...
                                published_on = dt.strftime("%Y-%m-%d %H:%M")
                            except Exception:
                                published_on = published_at
                            top_news.append(f"*{title}*\n{published_text} {published_on}\n[{read_more_text}]({url})")
                        return top_news
                    else:
                        logger.error(f"Error fetching Turkish news: HTTP {response.status}")
                        return []
        except Exception as e:
            logger.error(f"Error in Turkish market news: {e}")
            return []
    else:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(news_url) as response:
                    if response.status == 200:
                        ...
                        logger.error(f"Error fetching English news: HTTP {response.status}")
                        return []
        except Exception as e:
...
# --- COMMANDS WITH VADE SELECTION ---
# /start: dil seçimi ve hoş geldiniz mesajı
...
# /coin command: after user provides coin symbol, offer vade selection
async def coin(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text("Select term for analysis:", reply_markup=reply_markup)

# /long command: offer vade selection for long signals
async def long_signals(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text("Select term for long signals analysis:", reply_markup=reply_markup)

# /short command: offer vade selection for short signals
async def sell_signals(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text("Select term for short signals analysis:", reply_markup=reply_markup)

# /trend command: offer vade selection for trend analysis
async def trend(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text("Select term for trend analysis:", reply_markup=reply_markup)

# Callback query handler for analysis selections.
# Expected callback data format: "analysis:<type>:<symbol(optional)>:<term>"
async def analysis_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(parts) != 4:
        ...
        return
    analysis_type, subtype, symbol, term = parts[1], parts[1], parts[2], parts[3]
    lang = user_language.get(query.from_user.id, "en")
    if analysis_type == "coin":
        result_message, chart_image = await coin_analysis_by_term(symbol, term, lang, query.from_user.username)
        if chart_image:
            await query.edit_message_text(text=result_message)
            await context.bot.send_photo(chat_id=query.message.chat_id, photo=chart_image, caption=result_message, parse_mode="Markdown")
        else:
            await query.edit_message_text(text=result_message)
    elif analysis_type == "long":
        result_message = await long_signals_by_term(term, lang)
        await query.edit_message_text(text=result_message)
    elif analysis_type == "short":
        result_message = await short_signals_by_term(term, lang)
        await query.edit_message_text(text=result_message)
    elif analysis_type == "trend":
        result_text, chart_image = await trend_analysis_by_term(term, lang)
        if chart_image:
            await query.edit_message_text(text=result_text)
            await context.bot.send_photo(chat_id=query.message.chat_id, photo=chart_image, caption=result_text, parse_mode="Markdown")
        else:
            await query.edit_message_text(text=result_text)
    else:
        await query.edit_message_text("Unknown analysis type.")
...
# Additional commands for favorites and risk management
