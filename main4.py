...
async def send_trade_notification(context: ContextTypes.DEFAULT_TYPE, symbol: str, direction: str,
    ...
    # Compose the notification message with enhanced formatting
    message_text = (
        f"🪬 *Trade Signal*\n\n"
        f"📊 *Coin*: {symbol}\n"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_user_allowed(update):
        ...
        await update.message.reply_text(
            ...
            'To register: @paraloperceo\n'
            'Join Global Community: @paraloper'
        )
    ...
    try:
        ...
        if indicators:
            ...
            if signals['buy_signal']:
                signal_message += (
                    f"🚀 *Direction*: LONG\n"
                    f"💰 *Entry*: {indicators['entry_price']:.4f}\n"
                    f"🎯 *Take Profit*: {signals['tp_long']:.4f}\n"
                    f"🛡️ *Stop Loss*: {signals['sl_long']:.4f}\n"
                )
                direction = 'Long'
                ...
                signal_message += (
                    f"🩸 *Direction*: SHORT\n"
                    f"💰 *Entry*: {indicators['entry_price']:.4f}\n"
                    f"🎯 *Take Profit*: {signals['tp_short']:.4f}\n"
                    f"🛡️ *Stop Loss*: {signals['sl_short']:.4f}\n"
                )
                direction = 'Short'
                entry_price = indicators['entry_price']
                tp = signals['tp_short']
        ...
        if sell_signals_list:
            ...
            for symbol, strength, sig, entry_price in sell_signals_list:
                signals_message += (
                    ...
                    f"   🔀 Direction: SHORT\n"
                    f"   💰 Entry: {entry_price:.4f}\n"
                    f"   🎯 TP: {sig['tp_short']:.4f}\n"
                    f"   🛡️ SL: {sig['sl_short']:.4f}\n\n"
                )
