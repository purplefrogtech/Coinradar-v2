...
async def generate_trend_chart() -> io.BytesIO:
    """
    BTCUSDT günlük verilerini kullanarak, EMA 20 ve EMA 50 çizgilerini içeren trend analiz grafiğini oluşturur.
    """
...
async def trend(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    /trend komutuyla genel piyasa trend analizini yapar, sonucu metin olarak bildirir 
    ve BTCUSDT için oluşturulan trend analiz grafiğini gönderir.
    """
    ...
    try:
        ...
        if indicators:
            ...
            if risk_setting is not None and direction is not None:
                ...
                if risk_distance > 0:
                    # Yeni hesaplama: (entry_price * (risk_setting/100)) / risk_distance
...
# --- EK ÖZELLİKLER: GERÇEK ZAMANLI VERİ TAKİBİ, GELİŞMİŞ TEKNİK ANALİZ, PİYASA HABERLERİ ---

async def realtime(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    /realtime komutu: Belirtilen coin için gerçek zamanlı fiyat güncellemelerini verir.
    Kullanım: /realtime BTCUSDT
    """
    update_user_activity(update)
    if not is_user_allowed(update):
        await update.message.reply_text("You don't have access permission for the AI trader.")
...
async def generate_adv_chart(symbol: str, timeframe: str = "1h") -> io.BytesIO:
    """
    Belirtilen zaman diliminde, Ichimoku Bulutu, Stochastic Oscillator ve RSI ile gelişmiş teknik analiz grafiği oluşturur.
    """
    data = await get_crypto_data(symbol, interval=timeframe, limit=100)
    if data.empty:
        return None

    # Ichimoku Bulutu hesaplama
    try:
    ...
    # Grafik oluşturma: 3 panel (Fiyat/Ichimoku, Stochastic, RSI)
    fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(12, 12), sharex=True)

    # Fiyat ve Ichimoku Bulutu
    ax1.plot(data.index, data['close'], label="Close", color="blue")
...
async def adv_analysis(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    /adv_analysis komutu: Belirtilen coin için gelişmiş teknik analiz grafiği oluşturur.
    Kullanım: /adv_analysis BTCUSDT
    """
    update_user_activity(update)
    if not is_user_allowed(update):
        await update.message.reply_text("You don't have access permission for the AI trader.")
...
async def get_market_news() -> list:
    """
    CryptoCompare API'sı kullanarak en güncel piyasa haberlerini getirir.
    """
...
async def news(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    /news komutu: En güncel piyasa haberlerini Telegram mesajı olarak gönderir.
    """
    update_user_activity(update)
    if not is_user_allowed(update):
        await update.message.reply_text("You don't have access permission for the AI trader.")
